...
router.get("/", auth, async (req, res) => {
  ...
  let question = await QuestionSpanish.find().sort({
    createdAt: -1,
  });
...
router.post("/", auth, async (req, res) => {
  question = new QuestionSpanish(req.body);
  question
    .save()
    .then((resp) => {
...
router.put("/:id", auth, async (req, res) => {
  try {
    let question = await QuestionSpanish.findById(req.params.id);
...
router.delete("/:id", auth, async (req, res) => {
  try {
    let question = await QuestionSpanish.findByIdAndDelete(req.params.id);
    if (!question) {
